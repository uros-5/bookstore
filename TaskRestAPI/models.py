...
class Korisnici(AbstractUser):
    ...
    brojPoste = models.IntegerField(verbose_name="brojPoste", default=0)
...
class Narudzbine(models.Model):
    datumNarucivanja = models.DateTimeField(auto_now_add=True,verbose_name="datumNarucivanja",null=True,blank=True)
    ...
    def save(self, placeno=False,*args, **kwargs):
        super(Narudzbine, self).save(*args, **kwargs)
        if(placeno==True):
            Narudzbine.datumPrijema = timezone.now()
            Narudzbine.placeno = placeno
    def __str__(self):
        return str(self.id)
...
class Knjige(models.Model):
    ...
    ISBN = models.CharField(verbose_name="ISBN",max_length=925)
...
class StavkeNarudzbine(models.Model):
    kolicina = models.IntegerField(verbose_name="kolicina",default=0)
...
class OceneKnjiga(models.Model):
    ocena = models.IntegerField(verbose_name="ocena",default=0)
    korisnik = models.ForeignKey(Korisnici, verbose_name="korisnik", related_name="korisnik+",on_delete=models.CASCADE)
