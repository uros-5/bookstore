...
class Korisnici(models.Model):
    ...
    def get_from_req(request):
        return Korisnici.objects.filter(korisnik=User.objects.get(id=request.session["_auth_user_id"]))[0]
...
class Narudzbine(models.Model):
    ...
    def narudzbine_korisnik(self,korisnik_id):
        narudzbine = Narudzbine.objects.filter(korisnik=Korisnici.objects.get(korisnik_id= korisnik_id))
        korisnik_naruceno = {}
        for i in narudzbine:
...
class Knjige(models.Model):
    ...
    def for_korpa(knjiga,stavka):
        return {"isbn": knjiga.ISBN, "slika": str(knjiga.slika),
        "naslov": knjiga.naslov, "cena": round(float(knjiga.cena),2),
